...
int h264_extradata_to_annexb(const uint8_t *codec_extradata, const int codec_extradata_size, AVPacket *out_extradata, int padding)
{
  ...
  const uint8_t *extradata = codec_extradata + 4; //扩展数据的前4个字节无用
...
int h264_mp4toannexb(AVFormatContext *fmt_ctx, AVPacket *in, FILE *dst_fd)
{
  ...
  buf = in->data; // 帧数
  buf_size = in->size;
  buf_end = in->data + in->size; //data地址向后移 size
...
int main(int argc, char *argv[])
{
  ...
  {
    //本身命令 输入文件 输出文件 三个参数
    av_log(NULL, AV_LOG_ERROR, "params error  params should be more than three \n");
    return -1;
  }
  //获取文件
  ...
  {
    av_log(NULL, AV_LOG_ERROR, "src_filename or dst_filename is null \n");
    return -1;
  }
  // av_register_all();
  //open output file
